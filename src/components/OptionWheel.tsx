...
export interface OptionWheelRef {
  /** Move the wheel to a specific index with animation. */
  select: (index: number) => void;
}

interface OptionWheelProps {
  items?: string[];
  /** Render a custom node for each item. Receives the item label, index, and whether it's selected. */
  renderItem?: (label: string, index: number, selected: boolean) => ReactNode;
  defaultSelected?: number;
  onChange?: (index: number, item: string) => void;
  /** Fixed size of each wheel cell in pixels. Defaults to fontSize*spacing*rem when not supplied. */
  itemHeight?: number;
  textColor?: string;
  activeColor?: string;
  side?: 'left' | 'right';
...
const OptionWheel = forwardRef<OptionWheelRef, OptionWheelProps>(
  (
    {
      items = DEFAULT_ITEMS,
      renderItem,
    ...
    const rowH = itemHeight ?? Math.max(fontSize * spacing * remPx, 1);
    ...
    }, [items, itemHeight, fontSize, spacing, curve, tilt, blur, fade, minOpacity, side, loop, smoothing, applyTarget]);
        ...
        {items.map((label, index) => {
          ...
          return (
            <div
              ...
              className={`option-wheel__item${selected ? ' option-wheel__item--selected' : ''}`}
              onClick={() => handleItemClick(index)}
            >
              {renderItem ? renderItem(label, index, selected) : label}
            </div>
          );
        })}
      </div>
